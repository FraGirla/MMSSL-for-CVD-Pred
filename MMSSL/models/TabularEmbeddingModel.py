...
class TabularEmbeddingModel(nn.Module):
  ...
  def build_encoder(self, original_args: Dict) -> nn.Sequential:
    modules = [nn.Linear(original_args['input_size'], original_args['tabular_embedding_dim'])]
    for _ in range(original_args['encoder_num_layers']-1):
      modules.extend([nn.ReLU(), nn.Linear(original_args['tabular_embedding_dim'], original_args['tabular_embedding_dim'])])
    return nn.Sequential(*modules)

  def build_projector(self, original_args: Dict) -> nn.Sequential:
    modules = [nn.ReLU(), nn.Linear(original_args['tabular_embedding_dim'], original_args['projection_dim'])]
