...
class MultimodalSimCLR(pl.LightningModule):
  ...
  def training_step(self, batch: Tuple[List[torch.Tensor], List[torch.Tensor], torch.Tensor, torch.Tensor, List[torch.Tensor]], _, optimizer_idx: int) -> torch.Tensor:
    ...
    if optimizer_idx == 0:
      ...
      # Log metrics
      self.log("multimodal.train.top1", top1_accuracy_mm, on_step=True, on_epoch=False)
      self.log("multimodal.train.top5", top5_accuracy_mm, on_step=True, on_epoch=False)
      self.log("multimodal.train.f1_score", f1_score_mm, on_step=True, on_epoch=False)
      self.log("multimodal.train.auc", auc_mm, on_step=True, on_epoch=False)
    ...
    if optimizer_idx == 1:
      ...
      # Calculate F1 score
      f1_score_class = torchmetrics.F1Score(task='binary').to(device=y_hat.device)(y_hat, y)
  ...
  def validation_step(self, batch: Tuple[List[torch.Tensor], List[torch.Tensor], torch.Tensor, torch.Tensor, List[torch.Tensor]], batch_idx: int) -> torch.Tensor:
    ...
    self.log("multimodal.val.top1", top1_accuracy_mm, on_epoch=True, on_step=False)
    self.log("multimodal.val.top5", top5_accuracy_mm, on_epoch=True, on_step=False)
    self.log("multimodal.val.f1_score", f1_score_mm, on_epoch=True, on_step=False)
    self.log("multimodal.val.auc", auc_mm, on_epoch=True, on_step=False)
