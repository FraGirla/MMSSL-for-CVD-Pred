...
class TabularEncoder(nn.Module):
  ...
  def __init__(self, args) -> None:
    ...
    if args.tabular_checkpoint:
      loaded_chkpt = torch.load(args.tabular_checkpoint)
      ...
      # Build architecture
      self.input_size = args.tabular_input_size
  ...
  def build_encoder(self, args: Dict) -> nn.Sequential:
    ...
    for _ in range(args['tabular_encoder_num_layers']-1):
  ...
  def build_encoder_bn_old(self, args: Dict) -> nn.Sequential:
    modules = [nn.Linear(args.tabular_input_size, args.tabular_embedding_dim)]
    for _ in range(args.tabular_encoder_num_layers-1):
  ...
  def init_weights(self, m: nn.Module, init_gain = 0.02) -> None:
    ...
    if isinstance(m, nn.Linear):
      if self.args.tabular_init_strat == 'normal':
        nn.init.normal_(m.weight.data, 0, 0.001)
      elif self.args.tabular_init_strat == 'xavier':
        nn.init.xavier_normal_(m.weight.data, gain=init_gain)
      elif self.args.tabular_init_strat == 'kaiming':
        nn.init.kaiming_normal_(m.weight.data, a=0, mode='fan_in')
      elif self.args.tabular_init_strat == 'orthogonal':
