...
class EvalTabularDataset(Dataset):
  """
  Dataset of tabular data that generates two views, one untouched and one corrupted.
  The corrupted view had a random fraction is replaced with values sampled 
  from the empirical marginal distribution of that value
  """
  def __init__(self, data_path: str, labels_path: str, one_hot: bool=True, field_lengths_tabular: str=None):
  ...
  def __getitem__(self, index: int) -> Tuple[List[torch.Tensor], torch.Tensor]:
    ...
    item = uncorrupted_item, torch.tensor(self.labels[index].clone().detach(), dtype=torch.long)
    return item
